...
ipcMain.handle('map-drive', async (_, drive: { letter: string, target: string, name: string }) => {
  try {
    ...
    // 3. Añadir ejecución nativa en el inicio de Windows (Auto-Mount)
    const letterOnly = drive.letter.replace(':', '')
    await execAsync(`reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run" /v "VirtualDriveMapper_${letterOnly}" /d "C:\\Windows\\System32\\subst.exe ${drive.letter} \\"${drive.target}\\"" /f`)
...
ipcMain.handle('unmap-drive', async (_, letter: string) => {
  try {
    ...
    // Remover del registro (Etiqueta y Auto-Mount)
    ...
    try {
      await execAsync(`reg delete "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run" /v "VirtualDriveMapper_${letterOnly}" /f`)
    } catch (e) {
      // Ignore if doesn't exist
    }
